...
genomes used are list here \ref{suptab:source_data}. % add a supplemental table with names and versions
...
file conversion with gffread from \cite{trapnell2012differential}; % BUSCO, files generated with gffread
...
search are given in Section \ref{sec:dcnn} of the Supplements.
...
on this see Section \ref{sec:longer} of the Supplements or the source code.
...
the individual softmax values of each base. The figures in Section \ref{sec:overlapping} of the 
...
(e.g. {\it Arabidopsis lyrata}, \cite{rawat2015improving}; {\it Asparagus officinalis}, 
\cite{harkess2017asparagus}; and {\it Chenopodium quinoa}, \cite{jarvis2017genome})  
and a different HMM-based tool for many more (e.g. {\it Citrus clementina}, \cite{wu2018genomics}; 
{\it Capsella grandiflora}, \cite{slotte2013capsella};,...), which would potentially bias 
reference based comparisons of Helixer's and Augustus' performance. % examples chosen alphabetically and skipping ons where I wasn't sure from the methods
...
RNAseq mapped metric collection with PicardTools \citep{Picard2019toolkit} and custom downstream 
...
% sections
\begin{supsec}\label{sec:generalization}\end{supsec}
\begin{supsec}\label{sec:hyperparameters}\end{supsec}
\begin{supsec}\label{sec:longer}\end{supsec}
\begin{supsec}\label{sec:overlapping}\end{supsec}
\begin{supsec}\label{sec:training_data}\end{supsec}
\begin{supsec}\label{sec:dcnn}\end{supsec}
\begin{supsec}\label{sec:rnaseq_eval}\end{supsec}
