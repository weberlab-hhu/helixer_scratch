...
reference to RNAseq coverage data. RNAseq coverage provides support for an 
...
for intergenic annotations. Looking at selected subsequences \ref{fig:examples} % and supplemental figures.
...
Helixer vs Augustus and Helixer vs the reference) for all genomes \ref{}. % H vs A main, H vs R supp
...
any of either coverage or spliced coverage.
...
These patterns were consistent in direction but varied in magnitude in the individual species. % double check
RNAseq based comparison of Helixer and the reference was less clear cut.
-Helixer is good at differentiating ig, ref better at introns
-diffs between species. 
-Helixer is substantially better for Tcacao, with a "poor Sub Genic F1"
-taken together: competitive quality predictions, in some cases the mistakes really are in the reference / we wouldn't want 100\% accuracy against the reference
