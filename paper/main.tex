...
\caption{Caption, caption.}\label{fig:boxplot}
\end{figure}
...
\section{Results}
Figure \ref{fig:boxplot} shows the side-by-side comparison of the distribution of performances on all non-training genomes in the animal and plant case by Subgenic F1. In the case of Helixer, these scores represent cross-species predictions. We also compare the average performances of different configurations of Helixer with Augustus and a dilated CNN baseline in Table \ref{tab:results}. The results show a clear improvement over the AUGUSTUS both in median performance and spread. We also significantly outperform the dilated CNN baseline and demonstrated the effectiveness of two techniques used during inference with the usage of longer input lengths being enabled by our LSTM stack without any fully connected layers on top. To our knowledge, this technique was not used before with a model used for gene annotation.
...
\midrule
[0.7, 1.6, 1.2, 1.2] & [0.3, 1.0, 0.9, 0.3] &\\
\botrule
