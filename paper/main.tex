...
\author[Sample \textit{et~al}.]{Felix Stiehler\,$^{\text{\sfb 1}}$, Marvin Steinborn\,$^{\text{\sfb 1}}$, Stephan Scholz, Daniela Dey, Andreas P.M. Weber\,$^{\text{\sfb 1}}$, Alisandra K. Denton\,$^{\text{\sfb 1,}*}$}
...
We call the genomes that were ultimately not used during training from the original 
...
genomes are by far the largest set. We also did not find a noticeable difference in 
...
For the training itself, we divide each continuous genomic sequence into 20,000 bp 
...
selected two checkpoints each, specifically the checkpoint from the epoch with the best precision and the  % please check / epoch model / version something
...
Table S\ref{suptab:bioinfo_params} for details). The above was 
automated and the results visualized with the code here (https://github.com/weberlab-hhu/RNAsleek).
...
reference to RNAseq coverage data. RNAseq evaluation was performed for
three each animals and plants. RNAseq coverage provides support for an 
