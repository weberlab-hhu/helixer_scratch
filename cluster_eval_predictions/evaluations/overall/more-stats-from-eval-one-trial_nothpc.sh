...
if [[ $# -lt 2 ]]; then
	echo "Usage: ./more-stats-from-eval-one-trial_nothpc.sh trial_folder eval_file_name"
	exit
fi
...
phase_2=`cat $trial_folder/$eval_file_name |grep '| phase_2 '|sed 's/ //g'|cut -f5 -d'|'`
phase_none=`cat $trial_folder/$eval_file_name |grep '| no_phase '|sed 's/ //g'|cut -f5 -d'|'`


echo "$model,$dataset,$genicf1,$subgenicf1,$phase_2,$phase_none"
