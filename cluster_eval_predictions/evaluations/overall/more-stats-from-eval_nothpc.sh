...
if [[ $# -lt 2 ]]; then
	echo "Usage: ./more-stats-from-eval.sh main_folder output_file_name"
	exit
fi
here=`readlink -f $0`
here=${here%/more-stats-from-eval*sh}
main_folder=$1
output_file_name=$2

for i in `ls -d "$main_folder"/*/`;
do 
  $here/more-stats-from-eval-one-trial_nothpc.sh $i  $output_file_name
done
